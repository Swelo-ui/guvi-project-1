...
def generate_bank_account(bank: Dict[str, Any]) -> str:
    """
    ...
    # Select a prefix
    prefix = random.choice(prefixes) if prefixes and prefixes[0] else ""
    
    # Generate remaining digits
    remaining = length - len(prefix)
    account = prefix + ''.join([str(random.randint(0, 9)) for _ in range(remaining)])
    
    return account


def generate_ifsc_code(bank: Dict[str, Any]) -> str:
    """
...
def generate_upi_id(name: str, bank: Dict[str, Any]) -> str:
    """
    ...
    handle = random.choice(common_handles) if random.random() > 0.3 else bank_handle
...
def generate_card_details() -> Dict[str, str]:
    """
...
def generate_complete_financial_identity(first_name: str) -> Dict[str, Any]:
    """
    ...
    # Select a bank
    bank = random.choice(data['banks'])
    
    # Generate all financial data
    account_number = generate_bank_account(bank)
    ifsc_code = generate_ifsc_code(bank)
    upi_id = generate_upi_id(first_name, bank)
    ...
    return {
        'bank': {
            ...
            'ifsc': ifsc_code,
            'branch': f"{random.choice(data['cities'])['name']} Main Branch"
        },
        'upi': {
            'primary': upi_id,
